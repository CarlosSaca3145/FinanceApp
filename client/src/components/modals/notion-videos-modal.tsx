...
export function NotionVideosModal({ open, onOpenChange, onSelectVideoForBrand }: NotionVideosModalProps) {
  ...
  }, [videos, searchTerm, selectedNiche, selectedStatus, sortBy]);
              ...
              {filteredVideos.map((video, idx) => {
                ...
                return (
                  <div
                    ...
                    className="bg-card hover:border-indigo-500/50 transition-all duration-200 border border-border rounded-xl p-4 flex flex-col justify-between shadow-xs hover:shadow-md group"
                  >
                    <div>
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <Badge variant="outline" className="text-[11px] capitalize bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400 border-indigo-200 dark:border-indigo-900">
                          {video.status || "Planificado"}
                        </Badge>
                        {video.nicho && (
